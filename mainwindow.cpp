    ...
    , ui(new Ui::MainWindow)
{
    ...
    m_plot = new PlotManager(this);
    m_plot->setSize(QSize(500, 500));
    connect(this, &MainWindow::sig_PlotDataReady, this, &MainWindow::drawPlot);
}
...
void MainWindow::on_pb_start_clicked()
{
    ...
    auto findMax = [&](QVector<double> res){
                                                ...
                                                static const double DT = 1.0/FD;
                                                static const size_t POINT_NUMBER = static_cast<size_t>(1.0/DT);
                                                
                                                double t = 0.;
                                                for (auto i = 0; i < POINT_NUMBER; ++i)
                                                {
                                                    data.append(QPointF(t, res[i]));
                                                    t+= DT;
                                                }

                                                emit sig_PlotDataReady(data);
                                             };
...
void MainWindow::drawPlot(const QVector<QPointF> data)
{
    QString title = QString("Напряжение канала %1").arg(ui->cmB_numCh->currentIndex() + 1);
    emit m_plot->plot(data, title, "Время, с", "Напряжение, В");
}
